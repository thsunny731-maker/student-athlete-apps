...
**학생선수 스마트 출결 관리**(이하 '본 서비스')는 개인정보 보호법 제30조에 따라 정보주체의 개인정보를 보호하고 이와 관련한 고충을 신속하고 원활하게 처리할 수 있도록 하기 위하여 다음과 같이 개인정보 처리방침을 수립·공개합니다.
...
* **학생/학부모 회원 가입 및 관리**: 학급 구성원 및 소속 종목 식별, 출결 현황 파악, 교사의 피드백 제공
* **서비스 제공**: 대회 및 훈련 참가를 위한 학교장확인서 신청 및 승인 처리, 법정 출결 허용 잔여일수 자동 연산 및 이력 관리
...
### 제3조 (처리하는 개인정보 항목)
본 서비스는 학습 및 출결 관리를 위해 필요한 최소한의 개인정보만을 수집합니다.
* **수집 항목**: 구글 소셜 로그인 정보(이메일, 이름), 학생 이름, 소속 종목명, 학년, 반
...
* **인증 암호화**: 이용자의 인증(구글 소셜 로그인) 및 세션은 안전하게 토큰화되어 관리되며, 개발자(관리자)도 비밀번호를 알 수 없습니다.
* **해킹 등에 대비한 기술적 대책**: 보안 인증을 획득한 전문 클라우드 플랫폼(Google Firebase 및 Vercel)을 기반으로 운영되며, 전 구간 보안 통신(HTTPS)을 사용하여 데이터를 암호화하여 전송합니다.
